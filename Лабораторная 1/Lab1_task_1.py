...
class worker:
    ...
    def __init__(self, name: str = None, age: int = None, salary: float = None):
        """
        Создание объекта класса "worker":
        :param name: имя сотрудника (str)
        :param age: возраст сотрудника (int)
        :param salary: зарплата сотрудника (float)

        Примеры:
        >>> worker1 = worker("Ivan", 25, 95000)   # Инициализация объекта класса
        """
        self.add_data(name, age, salary)

    def add_data(self, name: str = None, age: int = None, salary: str = None):
        """
